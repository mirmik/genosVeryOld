...
#include "genos/gstl/utility.h"
	...
	void hassign(T1& a, const T2& b){*(typename std::remove_reference<T2>::type*)&a = b;};
	
	template<typename T1, typename T2>
	void hassign(T1& a, T2&& b){*(typename std::remove_reference<T2>::type*)&a = b;};
	
	template<size_t bias, typename T1, typename T2>
	void hassign_bias(T1& a, const T2& b){*(typename std::remove_reference<T2>::type*)((char*)&a + bias) = b;};
	
	template<size_t bias, typename T1, typename T2>
	void hassign_bias(T1& a, T2&& b){*(typename std::remove_reference<T2>::type*)((char*)&a + bias) = b;};
