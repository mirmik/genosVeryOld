	...
	{	
		...
		void set_proc_to_current_thread(atom_process* ap)
		{
			...
			list_move_tail(&ap->prc_list,&process_list);
		};
		
		void schedule(){
			int8_t max_priority = -1;
			int8_t low_thread_priority = -1;
			atom_process* max_priority_ready_proc = 0;
			atom_process* prroc, *prrroc;
			...
			//по ходу дела убиваем зомби
			list_for_each_entry_safe(prroc, prrroc, &process_list, prc_list) 
			{
				...
				if (max_priority < prroc->priority) 			
				if (!bits_mask_internal_or(prroc->prc_flags, (uint8_t)(PROC_WAIT | PROC_STOP | PROC_EXECUTED)))	
				{max_priority = prroc->priority; 
				max_priority_ready_proc = prroc;};
			};
			
			//Вычисляем приоритет старшей нити
			if (current_thread != -1)  low_thread_priority = thr_array[current_thread].proc->priority;
			else low_thread_priority = -1;
			
			//Логика, записывающая задачу в процесс 
			
			//Если старшая нить приоритетней выбранного процесса, запускаем старшую нить
			if (low_thread_priority > max_priority) 
			{
				dprcond(brsee,"br1");dln;
				//dstp;
				goto rethread;
			};			
			...
			if (low_thread_priority == max_priority) 
			if (max_priority_ready_proc != thr_array[current_thread].proc)
			{
				dprcond(brsee,"br2");
				set_proc_to_current_thread(max_priority_ready_proc);
				goto rethread;
			}
